...
func (r *userRepository) CreateProfile(profile *domain.Profile) error {
	...
	_, err := r.db.Exec(query, &profile.Id, &profile.Name, &profile.PhoneNumber, &profile.Bio, &profile.Profile_Picture_Url)
	if err != nil {
...
func (r *userRepository) GetProfile(id uint) (*domain.Profile, error) {
	...
	err := row.Scan(&profile.Id, &profile.Name, &profile.PhoneNumber, &profile.Bio,
